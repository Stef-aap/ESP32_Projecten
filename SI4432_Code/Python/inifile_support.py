...
[ 2.10 , '05-09-2010', 'Stef Mientki',
'Test Conditions:', (2,),
"""
- NoCase_Dict pickle added (by converting it to a normal dict)
"""],

[ 2.9 , '06-07-2009', 'Stef Mientki',
'Test Conditions:', (3,),
"""
- Read: very weird behavior, if a string was detected, it started eval()
        to translated tuples, e.g. the string "open" could do weird things.
        Now extra test on first character is "(".
"""],


[ 2.8 , '06-07-2009', 'Stef Mientki',
'Test Conditions:', (2,),
"""
- Remove_Section, now flushes the buffer to the file
"""],

[ 2.7 , '21-04-2009', 'Stef Mientki',
'Test Conditions:', (2,),
"""
- Read_Bool, Read_Integer allowed if Force_Strings = True
"""],

[ 2.6 , '08-04-2009', 'Stef Mientki',
'Test ConditionsD:\DATA_actueel\P25_TO_Analyzer:', (2,),
"""
- ini-object creation, printout of error information
"""],

[ 2.5 , '31-03-2009', 'Stef Mientki',
'Test Conditions:', (2,),
"""
- ini-object creation, Force_Strings added
- Read_String allowed if Force_Strings = True
- Get_Section, possibility to specify Section added
"""],

[ 2.4 , '23-03-2009', 'Stef Mientki',
'Test Conditions:', (2,),
_(0, """
- Write: wx.Colour now also saves the Alpha channel
""")],

[ 2.3 , '17-10-2008', 'Stef Mientki',
'Test Conditions:', (2,),
_(0, """
- Read, improved, emty string and False returned None, now they return correct
""")],

[ 2.2 , '20-08-2008', 'Stef Mientki',
'Test Conditions:', (2,),
_(0, """
- Read_Dict, added
- Write,     uses standard pickle for dictionaries
""")],

[ 2.1 , '20-05-2008', 'Stef Mientki',
'Test Conditions:', (1,),
_(0, """
- added writing of unicode strings
""")],

[ 2.0 , '07-01-2008', 'Stef Mientki',
'Test Conditions:', (1,),
_(0, """
- Major changes in setting flags of ConfigObj
- (might affect existing inifiles !!)
- All read procedures replaced by just one general "Read"
- All other read procedures marked as "depreciated"
for changing old inifiles:
    ==== old =====               ==== New =====
1.  Caption = Code Editor        Caption = 'Code Editor'
2.  Pos = '(118, 444)'           Pos = (118, 444)
3.  Color Off = 255, 0, 0, 255   Color Off = (255, 0, 0, 255)

1. make all not standard types (bool,int,float,list,tuple) a string
2. remove string quotes from tuples (not strictly necessary)
3. make lists or tuples of series values (not strictly necessary)
""")],

[ 1.3 , '06-01-2008', 'Stef Mientki',
'Test Conditions:', (1,),
_(0, """
- Read_List2, removed, didn't work very well (see comment below)
- Read_List, improved with
    - removing spaces at beginning and end of an value
    - translating True / False as value instead of string
""")],

[ 1.2 , '28-12-2007', 'Stef Mientki',
'Test Conditions:', (),
_(0, """
- Read_List2, improved reading of a list
""")],

[ 1.1 , '21-12-2007', 'Stef Mientki',
'Test Conditions:', (1,),
_(0, """
- when a inifile is opened, an empty section is created
  so we can test on "if ini:"
""")],

[ 1.0 , '15-11-2007', 'Stef Mientki',
'Test Conditions:', (1,),
_(0, """
- orginal release
""")],
...
class inifile ( ConfigObj ):
  def __init__ ( self, filename, Force_Strings = False ) :
    ...
    try:
      #print('inifile(ConfigObj).__init__: ', filename)
  ...
  def Remove_Section (self, section):
    ...
    # Because essential changes are made, we flush here
    self.write()
    self.Modified = False
  ...
  def Read (self, name, default = None ):
    ...
    if (isinstance ( line, bytes) and line.startswith( b'(' ) ) or \
  ...
  def ReadGvV (self, name, default = None ):
    if not ( self.Has_Section( self.Section ) ) : return default
    name = name.strip()
    line = self [ self.Section ].get ( name )
    return line
  ...
  def Read_Float (self, name, default = 0.0 ):
    ...
    ## Read doesn't work always correctly !!, so it's valid to use Read_Float
    if not ( self.Has_Section( self.Section ) ) : return default
    name = name.strip()
    try:
  ...
  def Write (self, name, value):
    ...
    if isinstance ( value, NoCase_Dict ) :
      # pickle of NoCase_Dict, reports an error,
      # therefor convert it first to a normal dict
      self [ self.Section ][ name ] = pickle.dumps ( dict ( value ) )
    elif isinstance ( value, dict ) :
      self [ self.Section ][ name ] = pickle.dumps ( value )
    elif (value is None) or isinstance(value, (bool, int, basestring, str, float, list, tuple)):
...
if __name__ == '__main__':
  ...
  if Test ( 1 ) :
    ...
    """
    filename = r'D:\Data_TO_exe\Akto_Transport.txt'
    if filename:
      print(' FILE:', filename)
      ini = inifile ( filename, Force_Strings = True )
      for Section in ini.Sections():
        print('[' + Section +']')
        Names_Values = ini.Get_Section ( Section )
  ...
  if Test ( 9 ) :
    '''
    filename = r'test_inifile.iniii'
    ini = inifile ( filename, Force_Strings = True )
    ini.Section = 'MiniFrame'
    A = ini.Read ( 'Tree_Expansion' )
    ini.Write ('aap', r'beer\hjdsad\dhasjd')
    print type(A),A
    ini.Close ()
    '''


    filename = r'D:\Dummy_RadQuest_Net\Dekkerswald\LVPK\dekkerswald_LVPK\protocol_NCSI\vraaglst\ncsi_2010.vli'
    ini = inifile ( filename ) #, Force_Strings = False )
    ini.Section = 'Vraag 20'
    line = '1/' +  str(1) + ' Skip'
    #line = '1/1 Skip'
    regel = ini.Read ( line, '' )
    print('===', regel)
