...
_CHM_Links = {
'<default>' : 'P:/Python26/Doc/Python264.chm',
'wx'        : 'P:/Python26/wxPython2.8 Docs and Demos/docs/wx.chm',
'stc'       : 'P:/Python26/wxPython2.8 Docs and Demos/docs/wx.chm',
'win32api'  : 'P:/Python26/Lib/site-packages/PyWin32.chm',
}

_CTRL_Links = {
'<default>' : 'P:/Python26/Doc/Python2662.chm',
}
...
class Class_URL_Viewer ( wx.Panel ):
  def __init__ ( self, Parent, ini = None, name = 'URL_Viewer' ) :
    ...
    MainPanel         ,PanelVer, 01
      P1              ,PanelHor, 000000001
        ...
        self.Combo    ,wx.ComboBox   ,style=wx.CB_DROPDOWN|wx.PROCESS_ENTER
      self.Panel      ,PanelVer, 11
        self.Html_IE  ,IEHtmlWindow_Ext
    ...
    if Platform_Windows :
      if Old_IEWIN :
        ...
        self.Html_IE.AddEventSink ( self )
  ...
  def _On_Button_Stop ( self, event ) :
    ...
    """
    if Platform_Windows and self.Html_IE.visible :
      self.Html_IE.Stop()
  ...
  def _On_Button_Refresh ( self, event ) :
    ...
    Refresh a IE page
    """
    #The newer version doesn't have visible
    #if Platform_Windows and self.Html_IE.visible :
    if Platform_Windows :
  ...
  def BeforeNavigate2 ( self, this, pDisp, URL, Flags,
    ...
    Save URL in history list.
    """
    print('IEWIN: BeforeNavigate2')
    self.SetCursor ( wx.StockCursor ( wx.CURSOR_WAIT ) )
    if not ( TargetFrameName                  ) and \
       not ( URL.startswith ( 'about:'      ) ) and \
       not ( URL.startswith ( 'res:'        ) ) and \
       not ( URL.startswith ( 'javascript:' ) ) :
      self._URL_In_History ( URL )  # if not in list add !!
      self.Combo.SetValue ( URL )
  ...
  def StatusTextChange(self, this, Text):
    ...
    So we can use this link when a new window is requested.
    """
    print('IEWIN: StatusTextChange')
    if Text :
      if Text.startswith ( 'http' ) :
        self.Open_In_New_Window = Text
...
# ***********************************************************************
# ***********************************************************************
from gui_support import My_Frame_Class
...
if __name__ == '__main__':
  ...
  """ Alternatives ???
   import win32gui
   import win32help
   helpfile = r'P:/Python/wxPython2.8 Docs and Demos/docs/wx.chm'
   ...
   import win32help
   if win32help.HtmlHelp(0,
    "%s/doc/catalog.chm::/catalog/%s/%s.htm" %
    (orangedir, self.category, self.__class__.__name__[2:]),
    win32help.HH_DISPLAY_TOPIC):
     return
  """
  ...
  if Test ( 4 ) :
    ...
    import win32help
    helpfile = r'P:/Python/wxPython2.8 Docs and Demos/docs/wx.chm'
    helpfile = r'D:/Data_Python_25/chm_help/numpy.chm'
    hwnd = win32gui.GetActiveWindow()
    a = win32help.HtmlHelp(hwnd, helpfile, win32help.HH_DISPLAY_TOC, None)
    # help file should now be displayed on top of active window
    #win32help.HtmlHelp(hwnd, '', win32help.HH_CLOSE_ALL, None)
